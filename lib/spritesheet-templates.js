...
function templater(data, options) {
  ...
  // If we have 3x retina parameters
  var retina3xSprites;
  var retina3xSpritesheet;
  var retina3xSpritesheetInfo;
  var retina3xGroups;
  var retina3xGroupsInfo;
  if (data.retina3x_sprites || data.retina3x_spritesheet || data.retina3x_groups) {
    // Verify we have all the data
    assert(data.retina3x_sprites && data.retina3x_spritesheet && data.retina3x_groups,
      'Expected `data.retina3x_sprites`, `data.retina3x_spritesheet`, and `data.retina3x_groups` to be provided. ' +
      'However, at least one of them was missing. Please provide all of them.');

    // Collect, clone, and normalize our data
    retina3xSprites = JSON.parse(JSON.stringify(data.retina3x_sprites));
    retina3xSpritesheet = JSON.parse(JSON.stringify(data.retina3x_spritesheet));
    retina3xSpritesheetInfo = data.retina3x_spritesheet_info ?
      JSON.parse(JSON.stringify(data.retina3x_spritesheet_info)) : {};
    retina3xSpritesheetInfo.name = retina3xSpritesheetInfo.name || 'retina-spritesheet';
    retina3xGroups = JSON.parse(JSON.stringify(data.retina3x_groups));
    retina3xGroupsInfo = data.retina3x_groups_info ? JSON.parse(JSON.stringify(data.retina3x_groups_info)) : {};
    retina3xGroupsInfo.name = retina3xGroupsInfo.name || 'retina-groups';

    // Map groups to their normal/retina counterparts
    // DEV: If `group.index` ever needs to split on a per-normal/retina basis, add `group.normal_index/retina_index`
    retina3xGroups.forEach(function getRetinaGroupSprites (retina3xGroup) {
      retina3xGroup.normal = sprites[retina3xGroup.index];
      retina3xGroup.retina = retina3xSprites[retina3xGroup.index];
    });

    // Define normalized variables for 3x retina sprites and its spritesheet (e.g. `px`)
    templater.escapeImage(retina3xSpritesheet);
    templater.ensureTemplateVariables(retina3xSpritesheet);
    retina3xSprites.forEach(function addExtraData (retina3xSprite) {
      templater.addSpritesheetProperties(retina3xSprite, retina3xSpritesheet);
      templater.ensureTemplateVariables(retina3xSprite);
    });
  }

  // console.log(data.retina3x_sprites)
  ...
  var retVal = template({
    ...
    // Add in 3x retina info
    retina3x_sprites: retina3xSprites,
    retina3x_spritesheet: retina3xSpritesheet,
    retina3x_spritesheet_info: retina3xSpritesheetInfo,
    retina3x_groups: retina3xGroups,
    retina3x_groups_info: retina3xGroupsInfo,
...
templater.addHandlebarsTemplate = function (name, tmplStr) {
  ...
  function templateFn(data) {
    ...
    // If we have 3x retina data, generate strings for it as well
    if (data.retina3x_sprites) {
      data.retina3x_sprites.forEach(function addHandlebarsVariables (retina3xSprite) {
        templater.ensureHandlebarsVariables(retina3xSprite, transformFn);
      });
    }
    if (data.retina3x_spritesheet_info) {
      templater.ensureHandlebarsVariables(data.retina3x_spritesheet_info, transformFn);
    }
    if (data.retina3x_groups) {
      data.retina3x_groups.forEach(function addHandlebarsVariables (retina3xGroup) {
        templater.ensureHandlebarsVariables(retina3xGroup, transformFn);
      });
    }
    if (data.retina3x_groups_info) {
      templater.ensureHandlebarsVariables(data.retina3x_groups_info, transformFn);
    }
